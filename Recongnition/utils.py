...
def test(net, criterion, batch_size):
    ...
    testset = FGVC_100.FGVC_100('data', train=True, transform=transform_test)
...
def val(net, batch_size):
    ...
    #                                            transform=transform_test)
    testset = FGVC_100.FGVC_100('data', train=True, transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=None, num_workers=4)
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        print(predicted_com.cpu())

        fpr, tpr, threshold = metrics.roc_curve(targets.data.cpu(), predicted_com.cpu())
        ...
        plt.savefig('/model_data1/roc.png')
